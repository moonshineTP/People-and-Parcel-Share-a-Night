...
class PartialSolution:
		...
		for(T,E)in enumerate(I.routes):
			...
			for(N,C)in enumerate(E[1:],start=1):
				if A.is_ppick(C):
					...
					D+=1
			Q=E[-1];R=M>1 and E[-1]==0;S={_F:Q,_G:F,_H:H,_K:D,_D:R};L.append(S)
		return J,K,G,L,D
	...
	def possible_expand(E,t_idx):
		...
		for I in E.remaining_pass_serve:J,K=A.pserve(I);D=A.D[G][J]+A.D[J][K];C.append((_I,I,D))
		for B in E.remaining_parc_pick:
			L=A.q[B-1]
			if F[_H]+L<=A.Q[H]:D=A.D[G][A.lpick(B)];C.append((_P,B,D))
		for B in F[_G]:D=A.D[G][A.ldrop(B)];C.append((_Q,B,D))
	...
	def check_expand(A,route_idx,kind,actid):
		...
		if D[_D]:return _A
		if B==_I:return C in A.remaining_pass_serve
		if B==_P:return C in A.remaining_parc_pick and D[_H]+F.q[C-1]<=F.Q[E]
		if B==_Q:return C in D[_G]
	...
	def apply_extend(A,route_idx,kind,actid,inc):
		...
		if G==_I:K,J=E.pserve(C);I.append(K);I.append(J);A.node_assignment[K]=B;A.node_assignment[J]=B;A.remaining_pass_serve.discard(C);D[_F]=J;D[_K]+=1;A.route_costs[B]+=inc;A.max_cost=max(A.max_cost,A.route_costs[B]);A.min_cost=min(A.route_costs);A.avg_cost=sum(A.route_costs)/A.problem.K;A.num_actions+=1;return
		elif G==_P:
			F=E.q[C-1]
			if D[_H]+F>E.Q[B]:raise ValueError(f"Taxi {B} capacity exceeded for parcel {C}.")
			H=E.lpick(C);D[_H]+=F;D[_G].add(C);A.remaining_parc_pick.discard(C);A.remaining_parc_drop.add(C)
		elif G==_Q:
			F=E.q[C-1]
			if D[_H]-F<0:raise ValueError(f"Taxi {B} load cannot be negative after dropping parcel {C}.")
			H=E.ldrop(C);D[_H]-=F;D[_G].discard(C);A.remaining_parc_drop.discard(C)
		else:raise ValueError(f"Unknown action kind: {G}")
		D[_F]=H;D[_K]+=1;I.append(H);A.node_assignment[H]=B;A.route_costs[B]+=inc;A.max_cost=max(A.max_cost,A.route_costs[B]);A.min_cost=min(A.route_costs);A.avg_cost=sum(A.route_costs)/A.problem.K;A.num_actions+=1
	def apply_return(A,t_idx):
		C=t_idx;D=A.routes[C];B=A.states[C]
		if B[_D]:return
		if B[_F]==0 and B[_K]>0:B[_D]=_C;return
		if B[_G]:raise ValueError(f"Taxi {C} must drop all loads before returning to depot.")
		E=A.problem.D[B[_F]][0];D.append(0);B[_F]=0;B[_K]+=1;B[_D]=_C;A.route_costs[C]+=E;A.max_cost=max(A.max_cost,A.route_costs[C]);A.min_cost=min(A.route_costs);A.avg_cost=sum(A.route_costs)/A.problem.K;A.num_actions+=1
	def reverse_action(A,t_idx):
		...
		if B.is_pdrop(D):
			...
			H=F[-1];K=B.D[H][I]+B.D[I][J];C[_F]=H;C[_K]-=1;C[_D]=_A;A.remaining_pass_serve.add(L);A.node_assignment[J]=-1;A.node_assignment[I]=-1;A.route_costs[G]-=K;A.max_cost=max(A.route_costs);A.min_cost=min(A.route_costs);A.avg_cost=sum(A.route_costs)/A.problem.K;A.num_actions-=1;return
		D=F.pop();H=F[-1];K=B.D[H][D];C[_F]=H;C[_K]-=1;C[_D]=_A
		if B.is_lpick(D):E=B.rev_lpick(D);C[_H]-=B.q[E-1];C[_G].discard(E);A.remaining_parc_pick.add(E);A.remaining_parc_drop.discard(E)
		elif B.is_ldrop(D):E=B.rev_ldrop(D);C[_H]+=B.q[E-1];C[_G].add(E);A.remaining_parc_pick.discard(E);A.remaining_parc_drop.add(E)
		elif D==0:0
...
def enumerate_actions_greedily(partial,width=_B,asymmetric=_C):
	...
	if not R:
		...
		for C in E:
			Y=A.states[C]
			if A.check_return(C):Z=B.D[Y[_F]][0];S.append((C,_R,0,Z))
		return S[:D]
	return R
def sample_from_weight(rng,weights):
	...
	if C<1e-10:B=rng.randrange(len(A))
		...
		for(F,G)in enumerate(A):
			...
			if E<=D:B=F;break
	return B
def repair_one_route(partial,route_idx,steps,T=_J,seed=_B,verbose=_A):
	...
	return A,C
def repair_operator(partial,repair_proba=_B,steps=_B,T=_J,seed=_B,verbose=_A):
...
def destroy_one_route(problem,route,route_idx,steps=10,verbose=_A):
	...
	while D<steps and len(C)>1:
		...
		D+=1
	return C,D
def destroy_operator(sol,destroy_proba,destroy_steps,seed=_B,t=_J,verbose=_A):
...
def greedy_solver(problem,partial=_B,verbose=_A):
	...
	while A.is_pending():
		...
		if not E:return _B,{_O:B,_L:time.time()-C,_M:A.num_actions-D,_N:'error'}
		F=E[0];apply_general_action(A,F)
	G=A.to_solution();H={_O:B,_L:time.time()-C,_M:A.num_actions-D,_N:_S};return G,H
def iterative_greedy_solver(problem,partial=_B,iterations=10000,destroy_proba=.53,destroy_steps=13,destroy_t=1.3,rebuild_proba=.29,rebuild_steps=3,rebuild_t=1.2,time_limit=3e1,seed=_B,verbose=_A):
	...
	if not B:return _B,{_L:time.time()-G,_N:'error'}
	H=B.max_cost;C=U[_M];L=H;M=0;N=0;O=0;P=_S;I=0
	for V in range(1,iterations+1):
		if K and time.time()>=K:P=_X;break
		...
		A,a=greedy_solver(D,partial=J,verbose=_A);L+=A.max_cost if A else 0;C+=a[_M];I+=1 if A else 0
		if A and A.max_cost<H:B=A;H=A.max_cost;M+=1
	b=time.time()-G;c={_O:I,_M:C,'improvements':M,'actions_destroyed':N,'actions_rebuilt':O,'average_cost':L/(I+1),_L:b,_N:P};return B,c
class TreeSegment:
...
def cost_decrement_intra_swap(partial,route_idx,a_idx,len_a,b_idx,len_b):
	...
	if B+H==C:
		...
		if E is not _B:G+=A[L][E]
	return F-G
def intra_swap_one_route_operator(partial,route_idx,steps=_B,mode=_E,uplift=1,seed=_B,verbose=_A):
	...
	def i():
		...
		if not A:return
		if M==_E:return A[0]
		elif M==_T:return max(A,key=lambda x:x[2])
		elif M==_U:return a.choice(A)
	P=0;Z=[_A]*R
	...
	while _C:
		...
		j(Q);k(Q);P+=1;Z[K]=_C
	return D,Z,P
def intra_swap_operator(partial,steps=_B,mode=_E,uplift=1,seed=_B,verbose=_A):
...
def inter_swap_route_pair_operator(partial,route_a_idx,route_b_idx,steps=_B,mode=_E,uplift=1,seed=_B,verbose=_A):
	...
	def d():
		A=list(c())
		if not A:return
		if N==_U:return W.choice(A)
		elif N==_T:return max(A,key=lambda x:x[4])
		else:return A[0]
	K=0;V=0;L=[_A]*A.K
	if M is _B:M=10**9
	...
	g();return B,L,K
def inter_swap_operator(partial,steps=_B,mode=_E,uplift=1,seed=_B,verbose=_A):
...
def relocate_from_to(partial,route_from_idx,route_to_idx,steps,mode,uplift=1,seed=_B,verbose=_A):
	...
	def T():
		...
		for(E,I)in enumerate(B[1:],start=1):
			if A.is_ppick(I):C=E+1;H.append((E,C,_I))
			elif A.is_lpick(I):
				N=A.rev_lpick(I);O=A.ldrop(N);C=M.get(O)
				if C is not _B and C>E:H.append((E,C,_V))
		...
		for(E,C,K)in H:
			...
			for(T,U)in R:
				...
				if L==_E:yield(J,F);return
				else:yield(J,F)
	def U():
		A=list(T())
		if not A:return
		if L==_U:return N.choice(A)
		elif L==_T:return max(A,key=lambda x:x[1][2])
		else:return A[0]
	...
	def X():
		...
		return I,C
	Y,Z=X();return C,Y,Z
def relocate_operator(partial,steps=_B,mode=_E,uplift=1,seed=_B,verbose=_A):
	...
	while D<B:
		...
		if not J:break
	return A,H,D
def _default_defense_policy(partial,seed=_B,verbose=_A):A=partial;B,C=iterative_greedy_solver(A.problem,A,iterations=1000,time_limit=3.,seed=113*seed if seed is not _B else _B,verbose=verbose);return B
def beam_enumerator(problem,swarm=_B,n_partials=20,n_return=10,r_intra=.55,r_inter=.75,f_intra=.1,f_inter=.1,cost_function=balanced_scorer,time_limit=3e1,seed=_B,verbose=_A):
	M=time_limit;L=problem;J=n_partials;D=verbose;C=swarm;K=time.time();E=L.num_actions
	if C is _B:C=PartialSolutionSwarm(solutions=[PartialSolution(problem=L,routes=[])])
	def F(value):return min(max(value,.0),_J)
	X=F(r_intra);Y=F(r_inter);Z=F(f_intra);a=F(f_inter);N=max(0,int(E*X));O=max(0,int(E*Y));P=max(1,int(E*Z));Q=max(1,int(E*a))
	def G(beam):beam.sort(key=lambda ps:cost_function(ps,seed=seed))
	def b(beam,use_intra,use_inter,depth,base_seed):
		G=use_inter;F=use_intra;C=base_seed;B=depth
		if not(F or G):return beam
		H,I=_B,_B;J=[];K=_A;L=_A
		for(M,N)in enumerate(beam):
			A=N
			if C is not _B:H=C+108*B+11*M;I=C+801*B+31*M+23
			if F:
				if D and not K:print(f"[BeamSearch] Depth {B}: Applied intra-swap operator.");K=_C
				A,E,E=intra_swap_operator(A,steps=_B,mode=_E,uplift=1,seed=H,verbose=_A)
			if G:
				if D and not L:print(f"[BeamSearch] Depth {B}: Applied inter-swap operator.");L=_C
				A,E,E=inter_swap_operator(A,steps=_B,mode=_E,uplift=1,seed=I,verbose=_A)
			J.append(A)
		return J
	def R(beam,aggressive=_A):
		A=[];D=3 if aggressive else _B
		for C in beam:
			E=enumerate_actions_greedily(C,width=D,asymmetric=_C)
			for F in E:
				B=C.copy();apply_general_action(B,F)
				if not any(B.is_identical(A)for A in A):A.append(B)
		G(A);A=A[:J];return A[:J]
	A=C.partial_lists;G(A);B=C.partial_lists[0].num_actions;S=0;T=_S
	while A:
		if time.time()-K>=M:
			T=_X
			if D:print(f"[BeamSearch] Time limit ({M:.2f}s) reached at depth {B}. Terminating search.")
			break
		if not A[0].is_pending():
			if D:print(f"[BeamSearch] All solutions in beam completed at depth {B}. Terminating search.")
			break
		U=P is not _B and B>=N and(B-N)%P==0;V=Q is not _B and B>=O and(B-O)%Q==0
		if U or V:A=b(A,U,V,depth=B,base_seed=seed);G(A)
		H=R(A,aggressive=_C)
		if not H:raise RuntimeError('Beam search stalled: no candidates generated.')
		if len(H)<J:H=R(A,aggressive=_A)
		A=H;B+=1;S+=1
		if D:
			I=[A.max_cost for A in A];c=10**(len(str(B))-1)
			if B%c==0 or B%100==0:print(f"[BeamSearch] Depth {B}. Max_cost range: {min(I)} - {max(I)}. Average max cost: {sum(I)/len(I):.2f}. Time elapsed: {time.time()-K:.2f}s.")
	G(A);A=A[:n_return];C=PartialSolutionSwarm(solutions=A);W={_O:S,_L:time.time()-K,_N:T}
	if D:print();print(f"[BeamSearch] Completed. Final beam size {len(A)}");print(f"[BeamSearch] Beam depth reached: {B}");print(f"[BeamSearch] Beam cost range: {C.min_cost} - {C.max_cost}");print(f"[BeamSearch] Average max cost: {C.avg_cost}");print(f"[BeamSearch] Time taken: {W[_L]:.4f} seconds");print(_W);print()
	return C,W
def beam_solver(problem,swarm=_B,n_partials=20,n_returns=10,r_intra=.55,r_inter=.75,f_intra=.05,f_inter=.1,cost_function=balanced_scorer,defense_policy=_default_defense_policy,time_limit=3e1,seed=_B,verbose=_A):
	G=defense_policy;D=seed;A=verbose;B,I=beam_enumerator(problem=problem,swarm=swarm,n_partials=n_partials,n_return=n_returns,r_intra=r_intra,r_inter=r_inter,f_intra=f_intra,f_inter=f_inter,cost_function=cost_function,time_limit=time_limit,seed=D,verbose=A)
	for(F,E)in enumerate(B.partial_lists):
		if E.is_pending():
			H=G(E,seed=D,verbose=_A)
			if H is not _B:B.partial_lists[F]=PartialSolution.from_solution(H)
	if A:print('[BeamSearch] Applying relocate operator to refine final beam solutions...')
	for(F,E)in enumerate(B.partial_lists):J,K,K=relocate_operator(E,steps=_B,mode=_E,uplift=1,seed=D,verbose=_A);B.partial_lists[F]=J
	if A:print('[BeamSearch] Relocate applied. Selecting best solution...')
	C=B.opt()
	if C is _B:
		if A:print('[BeamSearch] No complete solution found in beam swarm.');print('[BeamSearch] Attempting defense policy to complete best partial...')
		L=min(B.partial_lists,key=lambda ps:ps.max_cost);C=G(L,seed=D,verbose=A)
	if A:
		if C is not _B:print(f"[BeamSearch] Best solution found with max_cost: {C.max_cost}.");print(_W);print()
		else:print('[BeamSearch] No valid solution could be constructed.');print(_W);print()
	return C,I
def read_instance():
...
def main(verbose=_A):
	E=read_instance();C=E.num_nodes
	if C<=100:A,B,D=300,20,10
	elif C<=250:A,B,D=210,15,6
	elif C<=500:A,B,D=120,10,4
	elif C<=1000:A,B,D=50,5,3
	else:A,B,D=32,4,2
	F,G=beam_solver(problem=E,n_partials=A,n_returns=B,r_intra=.55,r_inter=.75,f_intra=.05,f_inter=.1,cost_function=balanced_scorer,defense_policy=_default_defense_policy,time_limit=24e1,seed=42,verbose=verbose);F.stdin_print()
