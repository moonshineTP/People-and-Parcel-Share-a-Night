"""
Curater for VRPLIB instances to SARP format.
References:
- VRPLIB format: /share_a_ride/precurated/format.txt
- SARPLIB format: /share_a_ride/data/README.md
"""
import random, os, re
...
def _curate_vrplib_to_sarp_text(
    ...
    base_capacity = 3 * avg_q

    factor_low = min(capacity_factor, 1 / capacity_factor)
    factor_high = max(capacity_factor, 1 / capacity_factor)
    
    Qlow = int(factor_low * base_capacity + 0.5)
    Qhigh = int(factor_high * base_capacity + 0.5)
    
    # Ensure Q is strictly larger than q
    if Qlow <= max_q:
        Qlow = max_q + 1
    if Qhigh < Qlow:
        Qhigh = Qlow
...
def _get_num_vehicles_from_sol(sol_path: str) -> Optional[int]:
    """
    Try to determine the number of vehicles (routes) from a .sol file.
    Expected format: 
    Route #1: ...
    Route #2: ... 
    ...
...
def write_sarp_from_vrplib(
    ...
    """
    vrplib_path = os.path.join("share_a_ride", "precurated", dataset_dir, vrplib_file)
    sol_file = vrplib_file.replace(".vrp", ".sol")
    sol_path = os.path.join("share_a_ride", "precurated", dataset_dir, sol_file)
    
    final_sarp_path = os.path.join("share_a_ride", "data", purpose, dataset_dir, sarp_path)

    with open(vrplib_path, "r", encoding="utf-8") as f:
        vrplib_text = f.read()

    # Determine num_vehicles
    num_vehicles = _get_num_vehicles_from_sol(sol_path)
    if num_vehicles is None:
        # Fallback 1: try parsing from filename (e.g. "A-n32-k5.vrp")
        match = re.search(r'-k(\d+)', vrplib_file, re.IGNORECASE)
        if match:
            num_vehicles = int(match.group(1))
        else:
            # Fallback 2: default
            print(f"Warning: Could not determine num_vehicles for {vrplib_file}, defaulting to 20")
            num_vehicles = 20

    inst = _parse_vrplib(vrplib_text)
    sarp_text = _curate_vrplib_to_sarp_text(
        inst, num_vehicles=num_vehicles, request_ratio=0.5,
        capacity_factor=1.2, seed=42
    )

    os.makedirs(os.path.dirname(final_sarp_path), exist_ok=True)
    with open(final_sarp_path, "w", encoding="utf-8") as f:
        f.write(sarp_text)

    print(f"Wrote curated SARP instance {vrplib_file} to {final_sarp_path}")
