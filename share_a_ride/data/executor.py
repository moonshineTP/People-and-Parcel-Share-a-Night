...
def attempt_dataset(
    ...
    final_summary = (
        f"Attempted {len(instance_files)} instances in dataset '{dataset}':\n"
    )
...
def attempt_instance(
    ...
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            f"CSV file for dataset '{dataset}' not found at {csv_path}."
        )
    ...
    try:
        ...
        hyperparams_json = (
            json.dumps(solver.hyperparams) if solver.hyperparams else "{}"
        )
